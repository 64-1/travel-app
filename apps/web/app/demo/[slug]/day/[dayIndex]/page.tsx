...
export default function DemoDayPage() {
  const { slug, dayIndex: dayIndexStr } = useParams<{ slug: string; dayIndex: string }>();
  ...
  if (!day) {
    return (
      <div className="py-12 text-center">
        ...
        <Link href={`/demo/${slug}/day/0`} className="text-primary mt-4 inline-block text-sm">
          {t("common.backHome")}
        </Link>
      </div>
    );
  ...
  return <ShareDayPanel day={day} trip={trip} basePath={`/demo/${slug}`} />;
}
